...
def jslint_check(files_to_check)
  ...
  growl system("jslint #{files_to_check}")
end
#END:JSLINT
...
def run_specs
  system('clear')
  path = "vendor/js/jasmine-node/lib:spec/javascripts:public/javascripts"
  system("env NODE_PATH=#{path} node vendor/js/jasmine-node/specs.js")
end
#END:JASMINE-METHOD
#END:MERGED
...
def run(files_to_run)
  ...
  system("clear")
  results = `rspec -cfs #{files_to_run}`
  growl results.split("\n").last rescue nil
  puts results
end
#END:SPECS
